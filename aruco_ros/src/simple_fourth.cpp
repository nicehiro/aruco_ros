// Description: This node publishes the pose of three markers in the same frame.
...
int main(int argc, char **argv)
{
  ...
  pose_pub1 = nh.advertise<geometry_msgs::PoseStamped>("pose1", 100);
  pose_pub2 = nh.advertise<geometry_msgs::PoseStamped>("pose2", 100);
  pose_pub3 = nh.advertise<geometry_msgs::PoseStamped>("pose3", 100);
  pose_pub3 = nh.advertise<geometry_msgs::PoseStamped>("pose4", 100);

  nh.param<double>("marker_size", marker_size, 0.05);
  nh.param<int>("marker_id1", marker_id1, 582);
  nh.param<int>("marker_id2", marker_id2, 88);
  nh.param<int>("marker_id3", marker_id3, 66);
  nh.param<int>("marker_id3", marker_id4, 200);
  ...
  ROS_INFO("ArUco node will publish pose to TF with (%s, %s), (%s, %s), (%s, %s) and (%s, %s) as (parent,child).", parent_name.c_str(),
           child_name1.c_str(), parent_name.c_str(), child_name2.c_str(), parent_name.c_str(), child_name3.c_str(), parent_name.c_str(), child_name3.c_str());
